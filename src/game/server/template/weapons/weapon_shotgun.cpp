...
class CWeaponShotgun : public CBaseSDKCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( CWeaponShotgun, CBaseSDKCombatWeapon );
...
public:
	...
	virtual const Vector& GetBulletSpread( void )
	{
		...
		if( GetOwner() && (GetOwner()->Classify() == CLASS_PLAYER_ALLY) )
		{
...
void CWeaponShotgun::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
	...
	// Commented due to missing HL2 code.
	/*
...
//-----------------------------------------------------------------------------
// Purpose:	When we shipped RH2, the shotgun weapon did not override the
